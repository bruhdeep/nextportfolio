...
(async function oneko() {
  ...
  function sleep() {
    ...
    if (spotify) {
      mousePosX = spotify.getBoundingClientRect().right - 32;
      mousePosY = spotify.getBoundingClientRect().top - 12;
      return;
    }
  }
  ...
  function frame() {
    ...
    // Cat has to sleep on spotify bar
    if (
      forceSleep &&
      Math.abs(diffY) < nekoSpeed &&
      Math.abs(diffX) < nekoSpeed
    ) {
      // Make the cat sleep exactly on the spotify bar
      nekoPosX = mousePosX;
      nekoPosY = mousePosY;
      nekoEl.style.left = `${nekoPosX - 16}px`;
      nekoEl.style.top = `${nekoPosY - 16}px`;
